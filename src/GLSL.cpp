...
Some modifications by Carlo Nicolini, July 2012
...
Initialization:
   ...
   if (shader==0)
      cerr << "GLSL Error Loading, compiling or linking shader\n";
...
char* aGLSLStrings[] =
{  "[Error 00] GLSL is not available!",
   "[Error 01] Not a valid program object!",
   "[Error 02] Not a valid object!",
   "[Error 03] Out of memory!",
   "[Error 04] Unknown compiler!",
   "[Error 05] Linker log is not available!",
   "[Error 06] Compiler log is not available!",
   "[Empty]"
};
...
{  if (extensions_init) return true;
    ...
    if (GLEW_OK != err)
    {  //cerr << "GLSL Error:" << glewGetErrorString(err) << endl;
        extensions_init = false;
        return false;
    }

    //cerr << "OpenGL Vendor: " << (char*) glGetString(GL_VENDOR) << "\n";
    //cerr << "OpenGL Renderer: " << (char*) glGetString(GL_RENDERER) << "\n";
    //cerr << "OpenGL Version: " << (char*) glGetString(GL_VERSION) << "\n\n";
    //cerr << "OpenGL Extensions:\n" << (char*) glGetString(GL_EXTENSIONS) << "\n\n";
...
{  InitOpenGLExtensions();
    ...
    if (!useGLSL)
    {  cerr << "**ERROR: OpenGL Shading Language is NOT available!" << endl;
    }
    else
...
{  if (!useGLSL) return false;
    ...
    {  glAttachShader(programObject, ShaderList[i]->ShaderObject);
        CHECK_GL_ERROR();
        //cerr << "attaching ProgramObj [" << i << "] @ 0x" << hex << ShaderList[i]->ProgramObject << " in ShaderObj @ 0x"  << ShaderObject << endl;
    }
...
{  if (!useGLSL) return aGLSLStrings[0];
    ...
    {  if (linker_log!=0)
        ...
        {  cerr << "GLSL Error: Could not allocate compiler_log buffer" << endl;
...
{  GLint loc;
    ...
    if (loc == -1)
    {  cerr << "GLSL Error: can't find uniform variable \"" << name << "\"\n";
    }
    CHECK_GL_ERROR();
    return loc;
}
...
{  glShader* o = new glShader();
    ...
    if (vertexFile!=0)
        ...
        {  cerr << "GLSL Error: can't load vertex shader!" << endl;
            delete o;
    ...
    if (fragmentFile!=0)
        ...
        {  cerr << "GLSL Error: can't load fragment shader!" << endl;
            delete o;
            delete tVertexShader;
            delete tFragmentShader;
            return 0;
    ...
    if (vertexFile!=0)
        ...
        {  cerr <<  "GLSL Vertex shader compiler"<< tVertexShader->getCompilerLog() << endl;
    ...
    if (fragmentFile!=0)
        ...
        {  cerr << "GLSL Fragment shader compiler" << tFragmentShader->getCompilerLog() << endl;
            delete o;
            delete tVertexShader;
            delete tFragmentShader;
            return 0;
    ...
    // link
    if (!o->link())
    {  cerr << "GLSL Linker error: " << o->getLinkerLog() << endl;
        delete o;
        delete tVertexShader;
        delete tFragmentShader;
        return 0;
...
{  glShader* o = new glShader();
    ...
    if (vertexFile!=0)
        if (tVertexShader->load(vertexFile) != 0)
        {  cerr << "GLSL Error: can't load vertex shader!\n";
            delete o;
            delete tVertexShader;
            delete tFragmentShader;
    ...
    if (geometryFile!=0)
        ...
        {  cerr << "GLSL Error: can't load geometry shader!\n";
            delete o;
            delete tVertexShader;
            delete tFragmentShader;
            delete tGeometryShader;
    ...
    if (fragmentFile!=0)
        if (tFragmentShader->load(fragmentFile) != 0)
        {  cerr << "GLSL Error: can't load fragment shader!\n";
            delete o;
            delete tVertexShader;
            delete tFragmentShader;
            delete tGeometryShader;
    ...
    {  if (!tGeometryShader->compile())
        {  cerr << "GLSL Geometry shader compiler : " << tGeometryShader->getCompilerLog() << endl;
...
{  glShader* o = new glShader();
    ...
    if (vertexMem!=0)
        if (!tVertexShader->compile())
        {  cerr << "GLSL VertexShader error: " << tVertexShader->getCompilerLog() << endl;
            delete o;
            delete tVertexShader;
            delete tFragmentShader;
            return 0;
    ...
    if (fragmentMem!=0)
        if (!tFragmentShader->compile())
        {  cerr << "GLSL Fragment error: " << tFragmentShader->getCompilerLog() << endl;
            delete o;
            delete tVertexShader;
            delete tFragmentShader;
            return 0;
